...
public abstract class AsyncTask<Params, Progress, Result> {
    ...
    private static class SmartSerialExecutor implements Executor {
        ...
        /**
         * 这里使用{@link ArrayDequeCompat}当栈比{@link Stack}性能高  创建一个数组 根据CPU设置容量大小
         */
        ...
        public synchronized void execute(final Runnable command) {
            ...
            //动态的监视线城池中的线程数 返回主动执行任务的近似线程数
            if (mCachedSerialExecutor.getActiveCount() < serialOneTime) {
                // 小于单次并发量直接运行  一个任务通过execute(Runnable)方法被添加到线程池，任务就是一个Runnable类型的对象，任务的执行方法就是Runnable类型对象的run()方法。
    ...
    public AsyncTask() {
        ...
        mWorker = new WorkerRunnable<Params, Result>() {
            public Result call() throws Exception {
                ...
                //在具体子类中实现doInBackground里的运算 并且post运算结果
